...
def everything_for_your_cat(cats_data):
    ...
    for lst in cats_data:
        k = " ".join(lst[2:])
        if k not in transit_dict:
            transit_dict[k] = str(lst[0]) + ', ' + str(lst[1])
        else:
            transit_dict[k] = transit_dict.get(k) + '; ' + str(lst[0]) + ', ' + str(lst[1])
    
    for key in transit_dict:
        our_str += key + ': ' + transit_dict.get(key) + '\n'
    return our_str
